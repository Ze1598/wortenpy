...
class Worten():
    ...
    Attributes:
        endpoint (str): The target website (worten.pt in this case).
        hitsPerPage (str): The number of results to be shown per page.
        page (str): The page of results to request.
        query (str): The query search term(s).
        query_soup (bs4.BeautifulSoup): A Beautiful Soup object holding the
    the source code for a query's page results.
        retrived_prods (list): A list of items where each item is the portion of
    the results page HTML source code relative to a product result.
        sortBy (str): The sorting order for the query results.
        total_num_prods_found (int): Number of product results for the query.
    '''

    # Simply connect to worten
    def __init__(self):
        '''
        Initialize the object by making a GET
        request to Worten.
    ...
    def make_query(self, tags, **kwargs):
        ...
        passed as keyword arguments, with string values.
        e.g.: make_query("god of war", sortBy="name", hitsPerPage="12", page="2")
        When the parameters are not present, sortBy defaults
        to "relevance", hitsPerPage to "24" and page to "1".
        For more information about the parameters, call the 
        query_params() class method.
        
        Args:
            tags (str): The string containing the tags for the query.
            sortBy (str) (optional): The sorting order for the query
        results.
            hitsPerPage (str) (optional): The number of results to be 
        shown per page.
            page (str) (optional): The page of results to request.
        ...
        self.total_num_prods_found = int(self.query_soup.find('span', class_='w-filters__element w-filters__results show-for-medium').text.split()[-1])
        return self.query_soup
    
    # Get source code for each product found    
    def get_prods(self, num_prods):
        '''
        Retrieve the first 'num_prods' products from the query 
        results (starting from the queued page, not the first page
        of results).
        
        Args:
            num_prods (int): The first 'num_prods' products will be
        retrieved.
        
        Return:
            retrieved_prods (list): A list of all the retrived
        products (rather, a list where each item is the portion of 
        the query's source code that's relative to the retrieved 
        products).
        '''
        ...
        while products_left:
            ...
            try:
                query_url_temp = 'https://www.worten.pt/search?' + urllib.parse.urlencode({'sortBy':self.sortBy, 'hitsPerPage':self.hitsPerPage, 'page':self.page, 'query':self.query})
    ...
    def get_prods_found(self, prods):
        '''
        Return a dictionary with mappings of the products'
        names found to the the URL of its pages.

        Args:
            prods (list): A list of snippets of HTML from
        the scraped source code of a query, where each snippet
        pertains to a specific result in a query.
        
        Returns:
            (dict): A dictionary with the mapping of
        product names and its pages' URLs.
        '''
        ...
        return {snippet.find('h3', class_='w-product__title').text: 'https://www.worten.pt' + snippet.find('a', class_='w-product__url qa-product-block__product-image--')['href'] for snippet in prods}        
    
    # Get query results page source code (current page)
    def get_prod_source(self):
        '''
        Get the BeautifulSoup object with the source code
        of the query results' current page.
        
        Args:
            None

        Returns:
            (requests.models.Response): The source code of the
        page.
        '''
        return self.query_soup
    ...
    def query_params(self):
        '''
        Returns a dictionary with information about each
        query parameter. The value for each pair is a tuple
        where the first item is a brief explanation of the 
        parameter, the second item is a list of the possible
        values for each parameter and the third item is the
        default value.
        ...
        Returns:
            (dict): The dictionary with the parameters' informations.
        '''
        return {
            'sortBy': ('How to sort the results', ['relevance', 'name', 'priceAsc', 'priceDesc', 'brand'], 'relevance'),
            'hitsPerPage': ('The number of results to show per page of results', ['12', '24', '48'], '24'),
            'page': ('The "landing page" of results of the query', ['An integer value'], '1')
        }
...
class Product():
    ...
    Attributes:
        all_info (dict): Dictionary with all the information 
    scraped about how a product.
        json_format (dict): A string to hold a JSON object 
    with the product's main information.
        more_info (bs4.element.Tag): The part of the product 
    page's source code for its detailed information.
        num_pics (int): Number of pictures available for the
    product.
        og_price (float): Original price for the product (in
    case it's on sale).
        pictures (list): A list with the URLs for the available
    product pictures.
        price_unit (str): Unit used for the product price.
        prod_avail (str): The product availability status.
        prod_brand (str): The product's brand.
        prod_cat (str): The product category.
        prod_color (str): The color of the product.
        prod_desc (str): The product description.
        prod_dimensions (str): Dimensions of the product.
        prod_discount (float): Discount percentage (in case
    the product is on sale).
        prod_ean (str): The product's European Article Number,
    also known as International Article Number.
        prod_link (str): Product page's URL.
        prod_model (str): The product model.
        prod_name (str): Name of the product.
        prod_price (float): The product's current price.
        prod_ref (str): The product internal reference.
        prod_weight (str): The product weight.
        query_prod (requests.models.Response): The response 
    for the GET request of a product's page.
        query_prod_soup (bs4.BeautifulSoup): Beautiful Soup 
    object for a product's page.
        xml_format (str):A string to hold a XML with the 
    ...
    def __init__(self, product):
        ...
        Args:
            product (bs4.element.Tag): The portion of the source
        code of a query made on Worten's website, relative to a 
        single product.
        ...
        # product to facilitate the work in other methods. After\
        # scraping each information, it'll be added to the dictionary
        self.all_info = {}
        ...
        # If something goes wrong, return None imediately
        except:
            return None

        # After connecting to the product's page, set a number of attributes for the object
        try:
            # Create a Beautiful Soup object for the source code of the product's page
        ...
        # Return None if it was not possible to create a BS object or scrape\
        ...
        # is scrape the original price and the discount percentage
        ...
        # If the product is not on sale, the attributes relative to discounts have value None
        else:
        ...
        # If the following tags are found, run the following lines,\
        # which is only run when a product has 5 or less pictures
        ...
        if self.query_prod_soup.find('div', class_='swiper-container w-product-gallery__thumbnails swiper__off'):
            pics = self.query_prod_soup.find('div', class_='swiper-container w-product-gallery__thumbnails swiper__off').find_all('div', class_='swiper-slide')
            self.pictures = ['https://www.worten.pt'+pic.a.img["src"] for pic in pics]
        # If the above tags are not found, try to find the tags for when\
        # a product has more than 5 pictures.
        # elif self.query_prod_soup.find('div', class_='swiper-wrapper').find_all('div'):
            ...
            self.pictures = ['https://www.worten.pt'+pic.a.img["src"] for pic in pics]
        # If neither of the above are found, assumed the product's\
        ...
        # Right column info (includes more specific information)
        ...
        # from the left column of the product specifications)
        ...
        for lst in l_info_LIs:
            ...
            for li in lst:
                # Get the label of the information of this <li>
                info_type = li.find('span', class_='details-label').text
                # If the label is on the translations dictionary, convert\
        ...
        # Dictionary to hold the main information scraped about the product
        self.prod_main_info = {
        ...
        # After setting up all of the main attributes, return None to finish __init__    
        return None
    ...
    def connected(self):
        ...
        successfully established. In other words, we are 
        testing if the request's status code is in the 200's.
    ...
    def on_sale(self):
        ...
        Boolean value from testing if the product has an
        ...
        Returns:
            (bool): True if the product has original price and
        discount percentage, else False.
        '''
    ...
    def get_main_info(self, *args):
        '''
        Return the scraped main information of a product.
        This can be a dictionary, a formated string
        to be written to an .xml file and/or a JSON
        object. The returned formats are saved in a single
        dictionary.

        Args:
            dictionary (str) (optional): Indicates that
        the method should return the information in the form
        of a Python dictionary.
            xml (str) (optional): Indicates that the
        method should return a formated string, ready to be
        written to an .xml file.
            json (str) (optional): Indicates that the
        method should return a JSON object.

        Returns:
            (dict): A dict that holds between zero to
        three pairs, with the possible items being the
        product's main information in the three different
        formats: Python dictionary, formated string for XML
        and/or a JSON object.
        '''
    ...
    def main_info_types(self):
        '''
        Returns a tuple with all the types of information about
        a product that are considered as main types of
        information.
        ...
        Returns:
            (tuple): The tuple with the main information types.
        '''
    ...
    def prod_info_types(self):
        ...
        methods to scrape specific informations about a
        product.
        ...
        Returns:
            (list): An alphabetically sorted list with all 
    ...
    def get_all_info(self, *args):
        '''
        Return all the scraped main information of a product.
        This can be a dictionary, a formated string
        to be written to an .xml file and/or a JSON
        object. The returned formats are saved in a single
        dictionary.
        ...
        Returns:
            (dict): A dict that holds between zero to
        three pairs, with the possible items being the
        product's information in the three different
        formats: Python dictionary, formated string for XML
        and/or a JSON object.
        '''
        # List to hold the created formats
    ...
    def specific_info(self, *args):
        ...
        an argument to the method call. If no argument
        was passed, return all of the "Extra 
        Information" scraped.
        For a full list of supported arguments, call
        the prod_info_types() method.

        Args:
            (list) (optional): A list of strings of the types
        of information to be returned as a dictionary

        Returns:
            (dict): A dictionary with key-value pairs of
        the type info_type: value.
        '''
...
if __name__ == '__main__':
    ...
    # print('Main information scraped:\n\t', single_prod.main_info_types(),'\n')
    # Example for how to print all the information scraped
    # print('Product scraped info:\n\t', [f'{key}: {single_prod.all_info[key]}' for key in single_prod.all_info], '\n')
